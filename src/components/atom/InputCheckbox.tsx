...
export const strokeColor = (color: string) => color.replace('#', '');

export const svgDataUrl = (size: number, url_14: string, url_16: string) => {
  return size === 14 ? url_14 : size === 16 ? url_16 : null;
};
...
export const InputCheckbox = styled.input<InputStyle>`
  ...
  background-image: ${({ size, theme }) =>
    `url(${setDefaultCheckbox(size, strokeColor(theme.color.black200))})`};
  :checked {
